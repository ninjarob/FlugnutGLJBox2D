...
import com.pkp.model.sprite.HelpText;

public class HelpScreen extends IScreen implements Swipeable{
	...
	public HelpScreen(IGame game) {
		...
		helpText = new HelpText(((GLGame) game), "helpText1.png");
		transitionIn = true;
	}
	...
	public void dispose() {
		...
		helpText.image.getTexture().dispose();
		if (nextScreen==null) {
	...
	public boolean transitionIn(float deltaTime) {
		return goBackButton.transitionIn(deltaTime) || helpText.transitionIn(deltaTime);
	}

	@Override
	public boolean transitionOut(float deltaTime) {
		return goBackButton.transitionOut(deltaTime) || helpText.transitionIn(deltaTime);
	}
}
